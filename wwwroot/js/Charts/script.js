...
// export async function handleOnLoad(data) {

//     Object.assign(disastersByFips, JSON.parse('data.json'));
...
export async function getCountyNameByFips(stateFips, countyFips) {
    ...
    try {
        ...
        const geoData = await response.json();  // Parse the JSON data

        // Loop through the GeoJSON features
        geoData.features.forEach(obj => {
            // Check if the state and county FIPS match and name is defined
            if (obj.properties.STATE === stateFips && obj.properties.COUNTY === countyFips && obj.properties.NAME !== undefined) {
                name = obj.properties.NAME;
            }
        });
